  ...
  return (
    <div className="fixed inset-0 bg-slate-900 flex flex-col overflow-hidden select-none animate-fade-in">
      ...
      <header className="p-8 md:p-12 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-800 bg-slate-900/50 backdrop-blur-md">
        ...
        <div className="flex gap-4">
          <button 
            ...
            className={`flex-1 md:flex-none px-10 py-5 rounded-3xl font-black text-xl transition-all flex items-center justify-center gap-3 ${activeTab === 'name' ? 'bg-blue-600 text-white shadow-2xl shadow-blue-600/30' : 'bg-slate-800 text-slate-400'}`}
          >
            <List size={28} />
            POR NOME
          </button>
          <button 
            ...
            className={`flex-1 md:flex-none px-10 py-5 rounded-3xl font-black text-xl transition-all flex items-center justify-center gap-3 ${activeTab === 'photo' ? 'bg-blue-600 text-white shadow-2xl shadow-blue-600/30' : 'bg-slate-800 text-slate-400'}`}
          >
            <Grid size={28} />
            POR FOTO
          </button>
      ...
      <main className="flex-1 p-8 md:p-12 overflow-y-auto custom-scrollbar flex flex-col">
        ...
        <div className="mb-10 relative">
          ...
          <input 
            ...
            className="w-full pl-20 pr-8 py-8 bg-slate-800 border-2 border-slate-700 rounded-[2rem] text-3xl font-bold text-white outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-900/20 transition-all placeholder:text-slate-600"
          />
        </div>
          ...
          <div className="space-y-4">
            {activeStudents.slice(0, 15).map(student => (
              <button 
                ...
                className="w-full p-8 bg-slate-800/50 hover:bg-slate-800 border border-slate-700/50 rounded-3xl flex items-center justify-between group transition-all active:scale-[0.98]"
              >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            {activeStudents.map(student => (
              <button 
                key={student.id}
                onClick={() => handleCheckIn(student)}
                className="flex flex-col items-center gap-4 bg-slate-800/40 p-6 rounded-[2.5rem] border-2 border-transparent hover:border-blue-500 active:scale-95 transition-all group"
              >
        ...
        {activeStudents.length === 0 && (
          <div className="flex-1 flex flex-col items-center justify-center text-center opacity-50 py-20">
            ...
            <p className="text-slate-600 mt-2 text-xl">Tente digitar um nome diferente ou limpe a busca.</p>
          </div>
        )}
      </main>
      ...
      {successMessage && (
        <div className="fixed inset-0 flex items-center justify-center z-[100] p-6">
          <div className="bg-green-600 text-white p-12 rounded-[3rem] shadow-2xl flex flex-col items-center gap-6 animate-zoom-in border-4 border-white/20">
            ...
            <p className="text-2xl font-bold opacity-80 uppercase tracking-widest">Bom Treino!</p>
          </div>
        </div>
      )}
      ...
      <button 
        ...
        className="fixed bottom-6 right-6 w-16 h-16 bg-slate-800/20 hover:bg-slate-800 flex items-center justify-center rounded-2xl transition-all group"
      >
